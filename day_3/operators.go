...
func solve(meal_cost float64, tip_percent int32, tax_percent int32) {
    ...
    meal_price_percent := meal_cost / 100
    tip := meal_price_percent * float64(tip_percent)
    tax := meal_price_percent * float64(tax_percent)
    total := tip + tax + meal_cost
    rounded_total_price := math.RoundToEven(total)
    fmt.Println(rounded_total_price)
}
